...
class InterFace(QWidget):
    ...
    def initUI(self):
        ...
        self.button_import4 = QPushButton('导出', self)
        ...
        self.button_import4.clicked.connect(self.export_data_to_xlsx)
    ...
    def export_data_to_xlsx(self):
        ...
        file_name = f"export_data_{current_time}.xlsx"
        ...
        # 自动调整所有列宽
        for col, column_data in enumerate(ws.columns, start=1):
            for row in column_data:
                ws.column_dimensions[openpyxl.utils.get_column_letter(col)].width = max(len(str(cell.value)) for cell in column_data if cell.value is not None)  # 计算列宽
